<?php

$page = [
    "title" => "Explore the collection",
    "topic_heading" => "Explore by topic",
    "topic_description" => "Discover highlights of The National Archives’ collections.",
    "time_heading" => "Explore by time period",
    "time_description" => "The National Archives contains over 1,000 years of British historical records. Select a time period to start exploring."
];

$generic_intro = [
    'heading' => $page['title'],
    'text' => "The National Archives holds over 24 million records relating to Britain and British history."
];

$breadcrumbs = [
    "Home" => "/"
];

$topic_card_data = [
    "Agriculture and environment" => [
        "description" => "Farming, food and royal parks",
        "url" => "/",
        "image" => "topics/cards/agriculture.png"
    ],
    "Arts and culture" => [
        "description" => "Photography, performing arts and sports",
        "url" => "/",
        "image" => "topics/cards/arts.png"
    ],
    "Business, finance and innovation" => [
        "description" => "Banking, taxation and computing",
        "url" => "/",
        "image" => "topics/cards/business.png"
    ],
    "Crime, courts and justice" => [
       "description" => "Pardons, pirates and prisons",
       "url" => "/",
       "image" => "topics/cards/crime.png"
    ],
    "Family and identity" => [
        "description" => "Census, children and gender",
        "url" => "/",
        "image" => "topics/cards/family.png"
    ],
    "Health and welfare" => [
        "description" => "Medicine, nursing and welfare",
        "url" => "/",
        "image" => "topics/cards/health.png"
    ],
    "International trade and affairs" => [
        "description" => "Slavery, trade and aid",
        "url" => "/",
        "image" => "topics/cards/international.png"
    ],
    "Land and buildings" => [
        "description" => "Construction, maps and manors",
        "url" => "/",
        "image" => "topics/cards/land.png"
    ],
    "Religion" => [
        "description" => "Discrimination, persecution and witchcraft",
        "url" => "/",
        "image" => "topics/cards/religion.png"
    ],
    "British state and citizens" => [
        "description" => "Policing, royalty and suffrage",
        "url" => "/",
        "image" => "topics/cards/state.png"
    ],
    "Transport and Travel" => [
        "description" => "Railways, roads and shipping",
        "url" => "/",
        "image" => "topics/cards/transport.png"
    ],
    "War" => [
        "description" => "Battles, weapons and refugees",
        "url" => "/",
        "image" => "topics/cards/war.png"
    ]
];


$time_card_data = [

    "Medieval (974 - 1485)" => [
        "description" => "Domesday Book, Magna Carta and the first printed document in England.",
        "url" => "/",
        "image" => "time/cards/medieval.png"
    ],
    "Early Modern (1485 - 1750)" => [
        "description" => "Elizabeth 1st’s seal, Drake’s letters and Shakespeare’s Will  ",
        "url" => "/",
        "image" => "time/cards/early-modern.png"
    ],
    "Empire and Industry (1750 - 1850)" => [
        "description" => "Dick Turpin’s indictment, Captain Cook’s journal and the US declaration of Independance ",
        "url" => "/",
        "image" => "time/cards/empire-and-industry.png"
    ],
    "Victorians (1850 - 1901)" => [
       "description" => "The Great Exhibition, Nelson’s funeral  and lthe trials of Oscar Wilde. ",
       "url" => "/",
       "image" => "time/cards/victorians.png"
    ],

    "Early 20th Century (1901 - 1918)" => [
        "description" => "SOS from the Titanic, Hitler’s passport and Windrush passenger lists.",
        "url" => "/",
        "image" => "time/cards/20th-century.png"
    ],
    "Interwar (1918 - 1939)" => [
        "description" => "Eden’s resignation, railway timetables and the home front.",
        "url" => "/",
        "image" => "time/cards/interwar.png"
    ],
    "Second World War (1939 - 1945)" => [
        "description" => "Dambusters operational records, Enigma engine and peace treaties. ",
        "url" => "/",
        "image" => "time/cards/wwii.png"
    ],
    "Postwar (945 - 2021)" => [
        "description" => "Cold war espionage, Poll tax riots and Brexit.",
        "url" => "/",
        "image" => "time/cards/postwar.png"
    ],
];
